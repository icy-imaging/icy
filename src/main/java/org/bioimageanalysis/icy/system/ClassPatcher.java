...
public class ClassPatcher {
    ...
    public ClassPatcher(final ClassLoader classLoader, final String patchPackage, final String patchSuffix) {
    ...
    public ClassPatcher(final String patchPackage, final String patchSuffix) {
     ...
     */
    public Class<?> loadClass(final String fullClass, final Class<?> neighbor) {
        final CtClass classRef = getClass(fullClass);
        try {
            //if (SystemUtil.getJavaVersionAsNumber() >= 9) // Always true now
            ...
            IcyLogger.error(ClassPatcher.class, e, "Cannot load class: " + fullClass);
            return null;
        }
    }
     ...
     * @return the loaded class
     */
    public Class<?> loadClass(final String fullClass, final Class<?> neighbor, final ClassLoader classLoader, final ProtectionDomain protectionDomain) {
        final CtClass classRef = getClass(fullClass);
        try {
            //if (SystemUtil.getJavaVersionAsNumber() >= 9) // Always true now
    ...
    /**
     * Generates a new line of code calling the imagej.legacy.patches class and method
    ...
    private String newCode(final String fullClass, final String methodSig) {
        ...
        final StringBuilder newCode = new StringBuilder((isVoid ? "" : "return ") + patchPackage + "." + className + patchSuffix + "." + methodName + "(");
        ...
        int i = 1;
        for (final String argName : getMethodArgNames(methodSig, true)) {
            if (firstArg)
                firstArg = false;
            else
                newCode.append(", ");
