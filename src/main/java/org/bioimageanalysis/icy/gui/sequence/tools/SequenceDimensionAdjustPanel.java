...
public class SequenceDimensionAdjustPanel extends JPanel {
     ...
     * Create the panel.
     */
    public SequenceDimensionAdjustPanel(final DimensionId dim) {
        super();
        ...
        if (dim == DimensionId.Z) {
            sequencePreviewPanel.getMainPanel().setBorder(new TitledBorder(null, "Select Z interval", TitledBorder.LEADING, TitledBorder.TOP, null, null));
        ...
        extractSpinner.addChangeListener(e -> {
        ...
        loopSpinner.addChangeListener(e -> fireRangeChangedEvent());
    }
    ...
    private void initialize() {
        ...
        final JPanel extractionRulePanel = new JPanel();
        ...
        final GridBagLayout gbl_extractionRulePanel = new GridBagLayout();
        ...
        final JLabel lblKeep = new JLabel("Keep");
        final GridBagConstraints gbc_lblKeep = new GridBagConstraints();
        ...
        extractSpinner.setModel(new SpinnerNumberModel(1L, 1L, Long.MAX_VALUE, 1L));
        extractSpinner.setToolTipText("Number of frames to keep every N frames. 'Keep 1 every 2' means keep every other frame.");
        final GridBagConstraints gbc_extractSpinner = new GridBagConstraints();
        ...
        final JLabel lblEvery = new JLabel("every");
        final GridBagConstraints gbc_lblEvery = new GridBagConstraints();
        ...
        loopSpinner.setModel(new SpinnerNumberModel(1L, 1L, Long.MAX_VALUE, 1L));
        loopSpinner.setToolTipText("Size for the extraction rule loop.");
        final GridBagConstraints gbc_loopSpinner = new GridBagConstraints();
    ...
    public void setPreviewVisible(final boolean value) {
    ...
    public boolean isIndexSelected(final int index) {
    ...
    protected void fireRangeChangedEvent() {
        for (final RangeChangeListener listener : getListeners(RangeChangeListener.class))
            listener.rangeChanged();
    }

    public void addRangeChangeListener(final RangeChangeListener listener) {
        listenerList.add(RangeChangeListener.class, listener);
    }

    public void removeRangeChangeListener(final RangeChangeListener listener) {
