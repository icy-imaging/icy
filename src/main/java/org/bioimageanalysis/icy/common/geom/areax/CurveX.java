...
public abstract class CurveX {
    ...
    public static void insertMove(final AreaXBody curves, final double x, final double y) {
        curves.add(new Order0X(x, y));
    }

    public static void insertLine(final AreaXBody curves, final double x0, final double y0, final double x1, final double y1) {
        if (y0 < y1) {
    ...
    public static void insertQuad(final CurveList curves, final double x0, final double y0, final double[] coords) {
        final double y1 = coords[3];
        if (y0 > y1) {
    ...
    public static void insertCubic(final CurveList curves, final double x0, final double y0, final double[] coords) {
        final double y1 = coords[5];
        if (y0 > y1) {
     ...
     * well.
     */
    public static int pointCrossingsForPath(final PathIterator pi, final double px, final double py) {
        if (pi.isDone()) {
            return 0;
        }
     ...
     */
    public static int pointCrossingsForLine(final double px, final double py, final double x0, final double y0, final double x1, final double y1) {
        if (py < y0 && py < y1)
            return 0;
        if (py >= y0 && py >= y1)
            return 0;
     ...
     */
    public static int pointCrossingsForQuad(final double px, final double py, final double x0, final double y0, double xc, double yc, final double x1, final double y1, final int level) {
     ...
     * crossing where the Y coordinate is decreasing
     */
    public static int pointCrossingsForCubic(final double px, final double py, final double x0,
                                             final double y0, double xc0, double yc0, double xc1, double yc1,
        ...
        xc1 = (xc1 + x1) / 2;
        yc1 = (yc1 + y1) / 2;
        final double xc0m = (xc0 + xmid) / 2;
        final double yc0m = (yc0 + ymid) / 2;
        final double xmc1 = (xmid + xc1) / 2;
        final double ymc1 = (ymid + yc1) / 2;
        xmid = (xc0m + xmc1) / 2;
        ymid = (yc0m + ymc1) / 2;
     ...
     * for NaN values.
     */
    public static int rectCrossingsForPath(final PathIterator pi, final double rxmin,
                                           final double rymin, final double rxmax, final double rymax) {
     ...
     */
    public static int rectCrossingsForLine(int crossings, final double rxmin,
                                           final double rymin, final double rxmax, final double rymax, final double x0, final double y0,
     ...
     * constant for more complete details.
     */
    public static int rectCrossingsForQuad(int crossings, final double rxmin,
                                           final double rymin, final double rxmax, final double rymax, final double x0, final double y0,
                                           double xc, double yc, final double x1, final double y1, final int level) {
     ...
     * constant for more complete details.
     */
    public static int rectCrossingsForCubic(int crossings, final double rxmin,
                                            final double rymin, final double rxmax, final double rymax, final double x0, final double y0,
                                            double xc0, double yc0, double xc1, double yc1, final double x1,
                                            final double y1, final int level) {
    ...
    public CurveX(final int direction) {
    ...
    public final CurveX getWithDirection(final int direction) {
    ...
    public static int orderof(final double x1, final double x2) {
        /*if (x1 < x2) {
            return -1;
        }
        if (x1 > x2) {
    ...
    public static long signeddiffbits(final double y1, final double y2) {
        return (Double.doubleToLongBits(y1) - Double.doubleToLongBits(y2));
    }

    public static long diffbits(final double y1, final double y2) {
        return Math.abs(Double.doubleToLongBits(y1) - Double.doubleToLongBits(y2));
    }

    public static double prev(final double v) {
        return Double.longBitsToDouble(Double.doubleToLongBits(v) - 1);
    }

    public static double next(final double v) {
        return Double.longBitsToDouble(Double.doubleToLongBits(v) + 1);
    }
    ...
    public int crossingsFor(final double x, final double y) {
    ...
    public boolean accumulateCrossings(final CrossingsX c) {
        final double xhi = c.getXHi();
        if (getXMin() >= xhi) {
            return false;
        }
        final double xlo = c.getXLo();
        final double ylo = c.getYLo();
        final double yhi = c.getYHi();
        final double y0 = getYTop();
        final double y1 = getYBot();
        double tstart;
        final double ystart;
        final double tend;
        final double yend;
    ...
    public CurveX getSubCurve(final double ystart, final double yend) {
    ...
    public int compareTo(final CurveX that, final double[] yrange) {
        /*
        ...
        final double scale = Math.max(Math.abs(y0), Math.abs(y1));
        final double ymin = Math.max(scale * 1E-14, 1E-300);
        if (fairlyClose(xs0, xt0)) {
            double bump = ymin;
            final double maxbump = Math.min(ymin * 1E13, (y1 - y0) * .1);
            double y = y0 + bump;
            while (y <= y1) {
         ...
         */
        while (s0 < s1 && t0 < t1) {
            final double sh = this.nextVertical(s0, s1);
            final double xsh = this.XforT(sh);
            final double ysh = this.YforT(sh);
            final double th = that.nextVertical(t0, t1);
            final double xth = that.XforT(th);
            final double yth = that.YforT(th);
            /*
        ...
        final double ymid = (yrange[0] + yrange[1]) / 2;
        /*
    ...
    public boolean findIntersect(final CurveX that, final double[] yrange, final double ymin,
                                 final int slevel, final int tlevel, final double s0, final double xs0, final double ys0,
                                 final double s1, final double xs1, final double ys1, final double t0, final double xt0,
                                 final double yt0, final double t1, final double xt1, final double yt1) {
        /*
        ...
        if (s1 - s0 > TMIN) {
            ...
            if (t1 - t0 > TMIN) {
                ...
                if (ys1 >= yt && yt1 >= ys) {
                    return findIntersect(that, yrange, ymin, slevel + 1, tlevel + 1, s, xs, ys, s1, xs1, ys1, t, xt, yt, t1, xt1, yt1);
                }
            }
            else {
                if (ys >= yt0) {
                ...
                if (yt1 >= ys) {
                    return findIntersect(that, yrange, ymin, slevel + 1, tlevel, s, xs, ys, s1, xs1, ys1, t0, xt0, yt0, t1, xt1, yt1);
                }
            }
        }
        else if (t1 - t0 > TMIN) {
            ...
            if (ys1 >= yt) {
                return findIntersect(that, yrange, ymin, slevel, tlevel + 1, s0, xs0, ys0, s1, xs1, ys1, t, xt, yt, t1, xt1, yt1);
            }
        }
        else {
            // No more subdivisions
            final double xlk = xs1 - xs0;
            final double ylk = ys1 - ys0;
            final double xnm = xt1 - xt0;
            final double ynm = yt1 - yt0;
            final double xmk = xt0 - xs0;
            final double ymk = yt0 - ys0;
    ...
    public double refineTforY(double t0, double yt0, final double y0) {
        double t1 = 1;
        while (true) {
            final double th = (t0 + t1) / 2;
            if (th == t0 || th == t1) {
                return t1;
            }
            final double y = YforT(th);
            if (y < y0) {
                t0 = th;
                yt0 = y;
            }
    ...
    public boolean fairlyClose(final double v1, final double v2) {
