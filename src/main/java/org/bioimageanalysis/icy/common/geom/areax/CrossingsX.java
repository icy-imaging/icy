...
public abstract class CrossingsX {
    ...
    public CrossingsX(final double xlo, final double ylo, final double xhi, final double yhi) {
        this.xlo = xlo;
        this.ylo = ylo;
        this.xhi = xhi;
        this.yhi = yhi;
    ...
    public void print() {
        ...
        builder.append("CrossingsX [\n\r  bounds = [").append(ylo).append(", ").append(yhi).append("]\n\r");
    ...
    public static CrossingsX findCrossings(final AreaXBody body, final double xlo, final double ylo, final double xhi, final double yhi) {
        final CrossingsX cross = new EvenOdd(xlo, ylo, xhi, yhi);
        for (int a = 0; a < body.size(); a++) {
            final CurveX c = body.get(a);
    ...
    public static CrossingsX findCrossings(final PathIterator pi, final double xlo, final double ylo, final double xhi, final double yhi) {
        final CrossingsX cross;
        ...
        final double[] coords = new double[23];
        double movx = 0;
        double movy = 0;
        double curx = 0;
        double cury = 0;
    ...
    public boolean accumulateLine(final double x0, final double y0, final double x1, final double y1) {
        if (y0 <= y1)
    ...
    public boolean accumulateLine(final double x0, final double y0, final double x1, final double y1, final int direction) {
        if (yhi <= y0 || ylo >= y1)
            return false;
    ...
    private final RawCurveArrayList tmp = new RawCurveArrayList(0);

    public boolean accumulateQuad(final double x0, final double y0, final double[] coords) {
    ...
    public boolean accumulateCubic(final double x0, final double y0, final double[] coords) {
    ...
    public final static class EvenOdd extends CrossingsX {
        public EvenOdd(final double xlo, final double ylo, final double xhi, final double yhi) {
            super(xlo, ylo, xhi, yhi);
        }

        @Override
        public boolean covers(final double ystart, final double yend) {
            return (limit == 2 && yranges[0] <= ystart && yranges[1] >= yend);
        }

        @Override
        public void record(double ystart, double yend, final int direction) {
            if (ystart >= yend) {
                return;
            }
            int from = 0;
            ...
            int to = from;
            while (from < limit) {
                final double yrlo = yranges[from++];
                final double yrhi = yranges[from++];
                if (yend < yrlo) {
            ...
            if (ystart < yend) {
                if (to >= yranges.length) {
                    final double[] newranges = new double[to + 10];
    ...
    public final static class NonZero extends CrossingsX {
        ...
        public NonZero(final double xlo, final double ylo, final double xhi, final double yhi) {
            super(xlo, ylo, xhi, yhi);
            crosscounts = new int[yranges.length / 2];
        }

        @Override
        public boolean covers(double ystart, final double yend) {
            int i = 0;
            while (i < limit) {
                final double ylo = yranges[i++];
                final double yhi = yranges[i++];
                if (ystart >= yhi) {
                    continue;
                }
                if (ystart < ylo) {
        ...
        public void remove(final int cur) {
            limit -= 2;
            final int rem = limit - cur;
            if (rem > 0) {
        ...
        public void insert(final int cur, final double lo, final double hi, final int dir) {
            final int rem = limit - cur;
            final double[] oldranges = yranges;
            final int[] oldcounts = crosscounts;
        ...
        @Override
        public void record(double ystart, final double yend, final int direction) {
            if (ystart >= yend) {
                return;
            }
            int cur = 0;
            ...
            if (cur < limit) {
                ...
                // assert(yrlo == ystart);
                final int newdir = rdir + direction;
                final double newend = Math.min(yend, yrhi);
                if (newdir == 0)
                    remove(cur);
                else {
