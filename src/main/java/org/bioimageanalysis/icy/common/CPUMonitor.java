...
public class CPUMonitor {
    private static class CPUTime {
        ...
        public void setStartUserTime(final long startUserTime) {
            this.startUserTime = startUserTime;
            setStopUserTime(startUserTime);
        }

        public void setStartCPUTime(final long startCPUTime) {
        ...
        public void setStopUserTime(final long stopUserTime) {
            this.stopUserTime = stopUserTime;
        }

        public void setStopCPUTime(final long stopCPUTime) {
    ...
    public CPUMonitor(final int type) {
        super();
    ...
    public void start() throws IllegalAccessError {
        ...
        switch (monitorType) {
            ...
            case MONITOR_ALL_THREAD_FINELY:
                threadTimes = new HashMap<>();
                time.setStartUserTime(0);
                time.setStartCPUTime(0);
                final long[] tids = bean.getAllThreadIds();
                for (final long id : tids) {
                    final CPUTime cput = new CPUTime();
    ...
    private long nanoToMilli(final long nano) {
        return Math.round(nano * NANO_TO_MILLI);
    }

    private double nanoToSec(final long nano) {
        return nano * NANO_TO_SEC;
    }

    private double milliToSec(final long milli) {
