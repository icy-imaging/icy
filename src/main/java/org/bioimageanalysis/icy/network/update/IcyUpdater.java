...
package org.bioimageanalysis.icy.network.update;
...
import org.bioimageanalysis.icy.network.update.ElementDescriptor.ElementFile;
...
public class IcyUpdater {
    ...
    static void showUpdateAndProcess(final ArrayList<ElementDescriptor> elements) {
        ...
        frame.setOkAction(e -> ThreadUtil.bgRun(() -> {
    ...
    static boolean prepareUpdate(final List<ElementDescriptor> elements, final boolean showProgress) {
        ...
        try {
            ...
            int curFile = 0;
            for (final ElementDescriptor element : elements) {
                for (final ElementFile elementFile : element.getFiles()) {
                    curFile++;
                    ...
                    if (downloadingFrame != null) {
                        ...
                        final String toolTip = "Downloading " + element.getName() + " : " + FileUtil.getFileName(elementFile.getLocalPath());
    ...
    public static boolean downloadAndSaveForUpdate(final String downloadPath, final String savePath, final ProgressFrame frame, final boolean displayError) {
        // get data
        final byte[] data;
     ...
     */
    public static boolean launchUpdater(final boolean doUpdate, final boolean restart) {
        if (doUpdate) {
