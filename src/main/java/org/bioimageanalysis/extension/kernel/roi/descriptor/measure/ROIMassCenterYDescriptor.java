...
public class ROIMassCenterYDescriptor extends ROIDescriptor {
    public static final String ID = "Mass center Y";
    ...
    public Object compute(final ROI roi, final Sequence sequence) throws UnsupportedOperationException, InterruptedException {
        return Double.valueOf(getMassCenterY(ROIUtil.computeMassCenter(roi)));
    }

    /**
     * Returns position Y of specified Point5D object
     */
    public static double getMassCenterY(final Point5D point) {
        if (point == null)
            return Double.NaN;
