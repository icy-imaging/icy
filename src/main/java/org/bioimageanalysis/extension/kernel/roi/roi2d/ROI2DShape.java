...
import org.bioimageanalysis.icy.model.overlay.anchor.Anchor2D.Anchor2DPositionListener;
...
import org.bioimageanalysis.icy.model.roi.edit.Point2DAddedROIEdit;
import org.bioimageanalysis.icy.model.roi.edit.Point2DMovedROIEdit;
import org.bioimageanalysis.icy.model.roi.edit.Point2DRemovedROIEdit;
...
public abstract class ROI2DShape extends ROI2D implements Shape {
    public class ROI2DShapePainter extends ROI2DPainter implements Runnable {
        ...
        protected final Set<Anchor2D> actorsToAdd;
        protected final Set<Anchor2D> actorsToRemove;
        ...
        protected void rebuildVtkObjects() {
            ...
            while (!path.isDone()) {
                switch (path.currentSegment(coords)) {
                    ...
                    case PathIterator.SEG_LINETO:
                        ...
                        polyList.add(new int[]{1 + ind, 2 + ind, ind}); // 0 + ind});
            ...
            try {
                ...
                vCells = newCells;
                // update outline data
                VtkUtil.setOutlineBounds(
                        outline,
                        bounds.getMinX() * xs, bounds.getMaxX() * xs,
                        bounds.getMinY() * ys, bounds.getMaxY() * ys,
                        z0, z1,
                        canvas
                );
        ...
        protected double getMinZ(final Sequence seq) {
        ...
        protected double getMaxZ(final Sequence seq) {
            final double curZ = getZ();
        ...
        public void drawROI(final Graphics2D g, final Sequence sequence, final IcyCanvas canvas) {
            ...
            if (canvas instanceof final VtkCanvas cnv) {
                ...
                synchronized (actorsToRemove) {
                    for (final Anchor2D anchor : actorsToRemove)
                ...
                synchronized (actorsToAdd) {
                    for (final Anchor2D anchor : actorsToAdd)
                        for (final vtkProp prop : anchor.getProps())
         ...
         * Draw the shape
         */
        protected void drawShape(final Graphics2D g, final Sequence sequence, final IcyCanvas canvas, final Shape shape, final boolean simplified) {
        ...
        /**
         * @return <code>true</code> if the specified bounds should be considered as "tiny" in the
        ...
        public void setColor(final Color value) {
            ...
            try {
                ...
                synchronized (controlPoints) {
                    for (final Anchor2D anchor : controlPoints) {
    ...
    public ROI2DShape(final Shape shape) {
        super();
        ...
        anchor2DPositionListener = this::controlPointPositionChanged;

        anchor2DOverlayListener = this::controlPointOverlayChanged;
    }
     ...
     * build a new anchor with specified position
     */
    protected Anchor2D createAnchor(final double x, final double y) {
    ...
    protected void updateShape() {
        ...
        try {
            ...
            synchronized (controlPoints) {
                for (final Anchor2D points : controlPoints)
                    points.setZ(z);
            }
        }
        finally {
     ...
     * Internal use only
     */
    protected void addPoint(final Anchor2D pt) {
        addPoint(pt, -1);
    }
     ...
     *        index where to add
     */
    public void addPoint(final Anchor2D pt, final int index) {
        // set Z position
        pt.setZ(getZ());
        // set visible state
     ...
     */
    public Anchor2D addNewPoint(final Point2D pos, final boolean insert, final boolean select) {
        if (!canAddPoint())
            return null;
     ...
     *         not support this operation for instance)
     */
    public Anchor2D addNewPoint(final Point2D pos, final boolean insert) {
     ...
     */
    public boolean removePoint(final Anchor2D pt) {
    ...
    protected void removeAllPoint() {
        synchronized (controlPoints) {
            ...
            synchronized (((ROI2DShapePainter) getOverlay()).actorsToAdd) {
                ...
                controlPoints.forEach(((ROI2DShapePainter) getOverlay()).actorsToAdd::remove);
            }
    ...
    public boolean removeSelectedPoint(final IcyCanvas canvas) {
        ...
        synchronized (controlPoints) {
            ...
            if (!controlPoints.isEmpty()) {
                ...
                if (selectedPoint instanceof final PathAnchor2D selectedPathPoint) {
    ...
    protected boolean selectPointAt(final IcyCanvas canvas, final Point2D imagePoint) {
        synchronized (controlPoints) {
     ...
     */
    protected int getInsertPointPosition(final Point2D pos) {
    ...
    @Override
    public boolean[] getBooleanMask(final int x, final int y, final int width, final int height, final boolean inclusive) {
        if ((width <= 0) || (height <= 0))
    ...
    public Rectangle2D computeBounds2D() {
        ...
        if (result.isEmpty()) {
            result.setFrame(result.getX(), result.getY(), Math.max(result.getWidth(), 0.001d), Math.max(result.getHeight(), 0.001d));
        }
    ...
    public void onChanged(final CollapsibleEvent object) {
        ...
        switch (event.getType()) {
            ...
            case PROPERTY_CHANGED:
                ...
                if (StringUtil.equals(property, PROPERTY_STROKE) || StringUtil.equals(property, PROPERTY_COLOR) || StringUtil.equals(property, PROPERTY_OPACITY))
                    ((ROI2DShapePainter) getOverlay()).updateVtkDisplayProperties();
                break;
