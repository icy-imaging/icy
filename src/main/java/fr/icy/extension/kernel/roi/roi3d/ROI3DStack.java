...
public class ROI3DStack<R extends ROI2D> extends ROI3D implements ROIListener, OverlayListener, Iterable<R> {
     ...
     */
    public ROI3DStack(final Class<? extends R> roiClass) {
        super();
    ...
    public void setColor(final int z, final Color value) {
     ...
     * @param z     z stack
     */
    public void setSlice(final int z, final R roi2d) {
        // nothing to do
        if (getSlice(z) == roi2d)
            return;
     ...
     * @throws UnsupportedOperationException if the given ROI slice cannot be added to this ROI
     */
    @SuppressWarnings("unchecked")
    public void add(final int z, final R roiSlice) throws UnsupportedOperationException, InterruptedException {
        if (roiSlice == null)
            return;
     ...
     * @param z        the position where the slice must be merged
     * @param roiSlice the 2D ROI to merge
     * @throws UnsupportedOperationException if the given ROI slice cannot be exclusively added to this ROI
     */
    @SuppressWarnings("unchecked")
    public void exclusiveAdd(final int z, final R roiSlice) throws UnsupportedOperationException, InterruptedException {
        if (roiSlice == null)
            return;
     ...
     * @param z        the position where the slice must be merged
     * @param roiSlice the 2D ROI to merge
     * @throws UnsupportedOperationException if the given ROI slice cannot be intersected with this ROI
     */
    @SuppressWarnings("unchecked")
    public void intersect(final int z, final R roiSlice) throws UnsupportedOperationException, InterruptedException {
     ...
     * @throws UnsupportedOperationException if the given ROI slice cannot be subtracted from this ROI
     */
    @SuppressWarnings("unchecked")
    public void subtract(final int z, final R roiSlice) throws UnsupportedOperationException, InterruptedException {
        if (roiSlice == null)
            return;
     ...
     */
    protected void sliceChanged(final ROIEvent event) {
    ...
    public double computeNumberOfContourPoints() throws InterruptedException {
        ...
        synchronized (slices) {
            if (slices.size() <= 2) {
                ...
                for (final R slice : slices.subMap(firstKey, false, lastKey, false).values())
     ...
     */
    public void translate(final int z) {
        // easy optimizations
        if ((z == 0) || isEmpty())
            return;

        synchronized (slices) {
            final Map<Integer, R> map = new HashMap<>(slices);

            slices.clear();
            for (final Entry<Integer, R> entry : map.entrySet()) {
    ...
    public boolean loadFromXML(final Node node) {
        ...
        try {
            ...
            for (final Element e : XMLUtil.getElements(node, "slice")) {
    ...
    public boolean saveToXML(final Node node) {
        ...
        synchronized (slices) {
            for (final R slice : slices.values()) {
                final Element sliceNode = XMLUtil.addElement(node, "slice");
