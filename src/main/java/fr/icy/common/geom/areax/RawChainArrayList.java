...
public class RawChainArrayList {
	 ...
	 * <code>elementData[0]</code> through <code>elementData[elementCount-1]</code> are
	 * the actual items.
	 */
	 ...
	 * argument.
	 * <p>
	 * If the current capacity of this list is less than <code>minCapacity</code>,
	 * then its capacity is increased by replacing its internal data array, kept
	 * in the field <code>elementData</code>, with a larger one. The size of the new
	 * data array will be the old size plus <code>capacityIncrement</code>, unless
	 * the value of <code>capacityIncrement</code> is less than or equal to zero, in
	 * which case the new capacity will be twice the old capacity; but if this
	 * new size is still smaller than <code>minCapacity</code>, then the new
