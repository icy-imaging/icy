 ...
 */
package fr.icy.model.colorspace;
...
import fr.icy.common.event.UpdateEventHandler;
...
public class IcyColorSpace extends ColorSpace implements ChangeListener, IcyColorMapListener {
     ...
     *
     * @param numComponents number of color component
     */
    public IcyColorSpace(final int numComponents) {
    ...
    public boolean hasAlphaComponent() {
        for (final IcyColorMap toRGBmap : toRGBmaps)
            if (toRGBmap.getType() == IcyColorMapType.ALPHA)
                return true;
    ...
    /**
     * @see ColorSpace#fromCIEXYZ(float[])
     */
    @Override
    ...
    public int toRGBUnnorm(final int[] colorvalue) {
        ...
        //return ((b > IcyColorMap.MAX_LEVEL) ? IcyColorMap.MAX_LEVEL : b) | (((g > IcyColorMap.MAX_LEVEL) ? IcyColorMap.MAX_LEVEL : g) << 8) | (((r > IcyColorMap.MAX_LEVEL) ? IcyColorMap.MAX_LEVEL : r) << 16) | (a << 24);
        return (Math.min(b, IcyColorMap.MAX_LEVEL)) | ((Math.min(g, IcyColorMap.MAX_LEVEL)) << 8) | ((Math.min(r, IcyColorMap.MAX_LEVEL)) << 16) | (a << 24);
    }
    ...
    public float[] toRGB(final float[] colorvalue) {
        ...
        result[0] = Math.min(bf, 1f);
        //result[1] = (gf > 1f) ? 1f : gf;
        result[1] = Math.min(gf, 1f);
        //result[2] = (rf > 1f) ? 1f : rf;
        result[2] = Math.min(rf, 1f);
        result[3] = af;
     ...
     *
     * @param unnormSrc source buffer containing unnormalized values ([0..255] range) for each component
     * @param dest      ARGB components buffer
     */
    public void fillARGBBuffer(final int[][] unnormSrc, final int[] dest, final int offset, final int length) {
        final int numComponents = getNumComponents();
     ...
     * @param dest      ARGB components buffer
     */
    public void fillARGBBuffer(final int[][] unnormSrc, final int[] dest) {
     ...
     * @param colorMap  source colorMap
     * @param setAlpha  also set the alpha information
     */
    public void setColorMap(final int component, final IcyColorMap colorMap, final boolean setAlpha) {
     ...
     */
    public FromRGBColorMap getFromRGBMap(final int component) {
     ...
     */
    public void setColorMaps(final ColorModel cm) {
     ...
     *
     * @param source   source colorspace to copy the colormaps from
     * @param setAlpha also set the alpha information
     */
    public void setColorMaps(final IcyColorSpace source, final boolean setAlpha) {
     ...
     * @return index
     */
    public int indexOfColorMap(final IcyColorMap colormap) {
     ...
     * Add a listener
     */
    public void addListener(final IcyColorSpaceListener listener) {
     ...
     * Remove a listener
     */
    public void removeListener(final IcyColorSpaceListener listener) {
     ...
     * fire event
     */
    public void fireEvent(final IcyColorSpaceEvent e) {
        for (final IcyColorSpaceListener listener : new ArrayList<>(listeners))
     ...
     */
    private void changed(final int component) {
        ...
        if (colorMap != null) {
            ...
            if (colorMap.getType() == IcyColorMapType.ALPHA) {
                ...
                for (final IcyColorMap map : toRGBmaps) {
                    if (map != colorMap) {
