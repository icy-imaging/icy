...
import fr.icy.model.roi.edit.PositionROIEdit;
...
public abstract class ROI3D extends ROI {
     ...
     */
    public static List<ROI3D> getROI3DList(final List<ROI> rois) {
        final List<ROI3D> result = new ArrayList<>();
    ...
    public abstract class ROI3DPainter extends ROIPainter {
        ...
        protected boolean updateDrag(final InputEvent e, final Point5D imagePoint, final IcyCanvas canvas) {
            ...
            // set new position
            setPosition3D(new Point3D.Double(startDragROIPosition.getX() + dx, startDragROIPosition.getY() + dy, startDragROIPosition.getZ() + dz));
         ...
         *        sequence
         */
        public void drawName(final Graphics2D g, final Sequence sequence, final IcyCanvas canvas) {
     ...
     */
    protected boolean onSamePos(final ROI3D roi, final boolean shouldContain) {
        final int t = getT();
        final int c = getC();
    ...
    @Override
    public boolean contains(final double x, final double y, final double z, final double t, final double c) {
        final boolean tok;
        final boolean cok;
    ...
    @Override
    public boolean contains(final double x, final double y, final double z, final double t, final double c, final double sizeX, final double sizeY, final double sizeZ,
                            final double sizeT, final double sizeC) {
        final boolean tok;
        final boolean cok;
    ...
    @Override
    public boolean intersects(final double x, final double y, final double z, final double t, final double c, final double sizeX, final double sizeY, final double sizeZ, final double sizeT, final double sizeC) {
        // easy discard
    ...
    public Rectangle5D computeBounds5D() {
        ...
        final Rectangle5D.Double result = new Rectangle5D.Double(
                bounds3D.getX(), bounds3D.getY(), bounds3D.getZ(), 0d, 0d,
                bounds3D.getSizeX(), bounds3D.getSizeY(), bounds3D.getSizeZ(), 0d, 0d
        );
    ...
    @Override
    public void setBounds5D(final Rectangle5D bounds) {
        beginUpdate();
        try {
            // infinite T dim ?
     ...
     *        new ROI 3D position
     */
    public void setPosition3D(final Point3D position) {
    ...
    @Override
    public void setPosition5D(final Point5D position) {
        beginUpdate();
        try {
    ...
    @Override
    public boolean[] getBooleanMask2D(final int x, final int y, final int width, final int height, final int z, final int t, final int c, final boolean inclusive) throws InterruptedException {
     ...
     */
    public boolean[] getBooleanMask2D(final int x, final int y, final int width, final int height, final int z, final boolean inclusive) throws InterruptedException {
     ...
     *        If true then all partially contained (intersected) pixels are included in the mask.
     */
    public boolean[] getBooleanMask2D(final Rectangle rect, final int z, final boolean inclusive) throws InterruptedException {
     ...
     *        If true then all partially contained (intersected) pixels are included in the mask.
     */
    public BooleanMask3D getBooleanMask3D(final int z, final int t, final int c, final boolean inclusive) throws InterruptedException {
        // not on the correct T, C position --> return empty mask
        if (!isActiveFor(t, c))
     ...
     */
    public double getSurfaceArea(final Sequence sequence) throws InterruptedException {
     ...
     */
    public boolean isActiveFor(final int t, final int c) {
     ...
     *        3D point
     */
    public boolean isOverEdge(final IcyCanvas canvas, final Point3D p) {
     ...
     *        5D Point
     */
    public boolean isOverEdge(final IcyCanvas canvas, final Point5D p) {
     ...
     *        double
     */
    public boolean isOverEdge(final IcyCanvas canvas, final double x, final double y, final double z, final double t, final double c) {
