...
public abstract class ROI2D extends ROI {
     ...
     */
    public static List<ROI2D> getROI2DList(final List<ROI> rois) {
        final List<ROI2D> result = new ArrayList<>();
    ...
    public abstract class ROI2DPainter extends ROIPainter {
        ...
        protected boolean updateFocus(final InputEvent e, final Point5D imagePoint, final IcyCanvas canvas) {
            ...
            final boolean focus = isOverEdge(canvas, imagePoint.getX(), imagePoint.getY());
        ...
        protected boolean updateDrag(final InputEvent e, final Point5D imagePoint, final IcyCanvas canvas) {
            ...
            final int dz = (getZ() == -1) || (imagePoint.getZ() == -1d) || (startDragMouseZ == -1) ? 0
     ...
     */
    public boolean isActiveFor(final int z, final int t, final int c) {
     ...
     */
    public boolean isOverEdge(final IcyCanvas canvas, final Point2D p) {
     ...
     */
    protected boolean onSamePos(final ROI2D roi, final boolean shouldContain) {
        final int z = getZ();
        final int t = getT();
        final int c = getC();
     ...
     *        new ROI 2D position
     */
    public void setPosition2D(final Point2D position) {
     ...
     */
    public boolean[] getBooleanMask(final int x, final int y, final int width, final int height, final boolean inclusive) throws InterruptedException {
     ...
     *        If true then all partially contained (intersected) pixels are included in the mask.
     */
    public boolean[] getBooleanMask(final Rectangle rect, final boolean inclusive) throws InterruptedException {
     ...
     *        If true then all partially contained (intersected) pixels are included in the mask.
     */
    public BooleanMask2D getBooleanMask(final boolean inclusive) throws InterruptedException {
     ...
     */
    @Override
    public ROI getSubROI(final int z, final int t, final int c) {
        if (!isActiveFor(z, t, c))
            return null;
