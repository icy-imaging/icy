...
public class MagicWand {
    public enum MagicWandConnectivity {
        ...
        public String toString() {
            return switch (this) {
                case CONNECT4 -> "4 ways";
                case CONNECT8 -> "8 ways";
            };
        }
    }
    ...
    public enum MagicWandGradientToleranceMode {
        ...
        public String toString() {
            return switch (this) {
                ...
                case P50 -> "50% of value tolerance";
                case P40 -> "40% of value tolerance";
                case P33 -> "33% of value tolerance";
                case P25 -> "25% of value tolerance";
                case P20 -> "20% of value tolerance";
                case P15 -> "15% of value tolerance";
                case P10 -> "10% of value tolerance";
                case P05 -> "5% of value tolerance";
            };
        }
    }
     ...
     *        -1 means color mode
     */
    public static ROI2D doWand2D(final Sequence sequence, final int xStart, final int yStart, final int z, final int t, final int channel, final double valueTolerance, final double colorSensitivity, final double gradientTolerance) {
        if (sequence == null)
            return null;
    ...
    public static ROI2D doWand2D(final Sequence sequence, final int xStart, final int yStart, final int z, final int t, final int channel, final MagicWandSetting mws) {
        ...
        final DataType dataType = img.getDataType();
        ...
        final boolean useGradient = (mws.gradientTolerance > 0d) && (mws.gradientTolerance < mws.valueTolerance);
        final double toleranceGrayGradTmp = mws.gradientTolerance;
        final double toleranceGrayGrad2 = sqr(toleranceGrayGradTmp);
        ...
        final int offset0 = getOffset(xStart, yStart, width);
        ...
        for (int iCoord = 0; iCoord <= lastCoord; iCoord++) {
                ...
                if (isInner) {
                    ...
                    for (int d = 0; d < 8; d++) {
                        if (isWithin(width, height, x, y, d)) {
                            final int x2 = x + dirXoffset[d];
                            final int y2 = y + dirYoffset[d];
                            final double v2 = getPixel(grayPixels, dataType, x2, y2, width);
                            // 2 for straight, 1 for diag
                            final int weight = (2 - (d & 0x1));
            ...
            for (int d = 0; d < 8; d += (mws.connectivity == MagicWandConnectivity.CONNECT4) ? 2 : 1) {
                ...
                final int offset2 = offset + dirOffset[d];
                ...
                if ((isInner || isWithin(width, height, x, y, d)) && maskPixels[offset2] == UNKNOWN) {
                    ...
                    if (!valueOK)
                        ...
                        if (lastCoord - iCoord > pixelPointerMask) {
                            ...
                            final int newSize = 2 * (pixelPointerMask + 1);
                            final int newMask = newSize - 1;
                            final int[] newPixelPointers = new int[newSize];
    ...
    public static ROI3D doWand3D(final Sequence sequence, final int xStart, final int yStart, final int zStart, final int t, final int channel, final double valueTolerance, final double colorSensitivity, final double gradientTolerance) {
        if (sequence == null)
            return null;
    ...
    public static ROI3D doWand3D(final Sequence sequence, final int x, final int y, final int z, final int t, final int channel, final MagicWandSetting mws) {
        return null;
    }

    private static int getOffset(final int x, final int y, final int w) {
        return (y * w) + x;
    }
    ...
    private static boolean checkColor(final int[] rgb, final int[] rgb0, final double[] rgbWeights, final MagicWandSetting mws) {
        final int r = rgb[0];
        final int g = rgb[1];
        final int b = rgb[2];
        final int r0 = rgb0[0];
        ...
        if (mws.colorSensitivity < 0) {
            ...
            final double deltaGray = (deltaR * rgbWeights[0]) + (deltaG * rgbWeights[1]) + (deltaB * rgbWeights[2]);
        ...
        final double deltaPerpSqr; // = 0;
        final double deltaPerpSqrFactor; // = 0;
     ...
     */
    private static boolean isWithin(final int w, final int h, final int x, final int y, final int direction) {
        final int xmax = w - 1;
        final int ymax = h - 1;

        return switch (direction) {
            default -> false; // should never occur, we use it only for directions 0-7
            case 0 -> (y > 0);
            case 1 -> (x < xmax) && (y > 0);
            case 2 -> (x < xmax);
