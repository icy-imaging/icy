...
import fr.icy.io.FileImporter;
import fr.icy.io.FileUtil;
import fr.icy.io.Loader;
import fr.icy.io.SequenceFileImporter;
import fr.icy.gui.dialog.LoaderOptionPanel.LoaderLoadingType;
import fr.icy.Icy;
    ...
    public LoaderDialog(String defaultPath, Rectangle region, int series, boolean autoLoad)
    {
        ...
        if (value == JFileChooser.APPROVE_OPTION)
        {
            ...
            if (autoLoad)
            {
                ...
                // all files filter selected ? --> find if we try to open image file or not
                // so we can load protocol correctly when 'all files filter' is selected
                if (importer == null)
                {
                    // multiple files ? test on several files (4 should be enough)
                    if (paths.size() > 0)
                    {
                        ...
                        for (int i = 0; i < paths.size(); i += inc)
                        {
                            ...
                            {
                                // so we have image, can stop here
                                notAnImage = false;
                                break;
                            }
                        }
                        ...
                        notAnImage = !Loader.isSupportedImageFile(firstPath);
                }
