...
public class AreaXBody extends CurveList
{
    ...
    public int getRelationship(AffineTransform lhsTransform, Shape rhs, AffineTransform at)
    {
        ...
        while (rhsi.isDone() == false)
        {
            ...
            if (i == PathIterator.SEG_MOVETO && lastX != moveX && lastY != moveY && closed == false)
            {
                if (intersectsLine(lastX, lastY, moveX, moveY, axi, lhsTransform))
                    return AreaX.RELATIONSHIP_COMPLEX;
            }
            ...
            if (i == PathIterator.SEG_MOVETO)
            {
                ...
                int newRelationship = contains ? AreaX.RELATIONSHIP_LHS_CONTAINS : AreaX.RELATIONSHIP_NONE;
                if (relationship == -1)
                {
            ...
            {
                if (intersectsLine(lastX, lastY, coords[0], coords[1], axi, lhsTransform))
                    return AreaX.RELATIONSHIP_COMPLEX;
                lastX = coords[0];
                lastY = coords[1];
            }
            ...
            {
                if (intersectsQuad(lastX, lastY, coords[0], coords[1], coords[2], coords[3], axi, lhsTransform))
                    return AreaX.RELATIONSHIP_COMPLEX;
                lastX = coords[2];
                lastY = coords[3];
            }
            ...
            {
                if (intersectsCubic(lastX, lastY, coords[0], coords[1], coords[2], coords[3], coords[4], coords[5], axi, lhsTransform))
                    return AreaX.RELATIONSHIP_COMPLEX;
                lastX = coords[4];
                lastY = coords[5];
            }
        ...
        if (relationship == AreaX.RELATIONSHIP_NONE)
        {
            ...
            for (int a = 0; a < elementCount; a++)
            {
                ...
                {
                    int newRelationship;
                    if (rhs.contains(elementData[a].getX0(), elementData[a].getY0()))
                    {
    ...
    private boolean intersectsCubic(double x0, double y0, double cx0, double cy0, double cx1, double cy1, double x1, double y1, AreaXIterator axi,
            AffineTransform axiTx)
    {
        axi.reset(this, axiTx);
        double lastX = 0;
        double lastY = 0;
        ...
        while (axi.isDone() == false)
        {
            ...
            {
                if (Intersections.cubicCubic(x0, y0, cx0, cy0, cx1, cy1, x1, y1, lastX, lastY, coords[0], coords[1], coords[2], coords[3], coords[4],
                        coords[5]))
                    return true;
                lastX = coords[4];
                lastY = coords[5];
        ...
        if (Math.abs(t.getShearX()) < .0000001 && Math.abs(t.getShearY()) < .0000001)
        {
            ...
            for (int a = 0; failed == false && a < size; a++)
            {
                ...
                else if (curve instanceof Order1X)
                {
                    ...
                    {
                        newCurve = new Order1X(x0, y0, x1, y1, curve.getDirection());
                    }
                    else
                    {
                        newCurve = new Order1X(x1, y1, x0, y0, curve.getDirection());
                    }
                }
                else if (curve instanceof Order2X)
                {
                    ...
                    if (curve.getDirection() == increasing)
                    {
                        newCurve = new Order2X(x0, y0, cx0, cy0, x1, y1, curve.getDirection());
                    }
                    else
                    {
                        newCurve = new Order2X(x1, y1, cx0, cy0, x0, y0, curve.getDirection());
                    }
                }
                else
                { // if(curve instanceof Order3X) {
                    ...
                    if (curve.getDirection() == increasing)
                    {
                        newCurve = new Order3X(x0, y0, cx0, cy0, cx1, cy1, x1, y1, curve.getDirection());
                    }
                    else
                    {
                        newCurve = new Order3X(x1, y1, cx1, cy1, cx0, cy0, x0, y0, curve.getDirection());
                    }
                }
                if ((newCurve.getY0() == newCurve.getY1()) != (curve.getY0() == curve.getY1()))
                {
                    /**
    ...
    public void next()
    {
        ...
        else
        {
            ...
            if (index < curves.size())
            {
                ...
                if (curve.getOrder() != 0 && prevcurve.getX1() == curve.getX0() && prevcurve.getY1() == curve.getY0())
                {
                    prevcurve = null;
                }
            }
