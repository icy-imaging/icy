...
public abstract class AreaOpX {
    public static abstract class CAGOp extends AreaOpX {
        ...
        public int classify(final EdgeX e) {
            ...
            final boolean newClass = newClassification(inLeft, inRight);
    ...
    public static class AddOp extends CAGOp {
        @Override
        public boolean newClassification(final boolean inLeft, final boolean inRight) {
    ...
    public AreaXBody calculate(AreaXBody left, AreaXBody right) {
        ...
        final RawEdgeArrayList edges = new RawSortedEdgeArrayList(left.size() + right.size(), YXTopComparator);
        addEdges(edges, left, AreaOpX.CTAG_LEFT);
        addEdges(edges, right, AreaOpX.CTAG_RIGHT);
        return pruneEdges(edges);
    }

    private static void addEdges(final RawEdgeArrayList edges, final AreaXBody curves, final int curvetag) {
        for (int a = 0; a < curves.size(); a++) {
            final CurveX curve = curves.get(a);
            if (curve.getOrder() > 0)
                edges.add(new EdgeX(curve, curvetag));
        }
    }

    private static final Comparator<EdgeX> YXTopComparator = (o1, o2) -> {
    ...
    private AreaXBody pruneEdges(final RawEdgeArrayList edges) {
        final int numedges = edges.size();
        if (numedges < 2) {
            return new AreaXBody(null, 0);
        }
        final EdgeX[] edgelist = edges.getArray();
        EdgeX e;
        int left = 0;
        int right = 0;
        int cur; // = 0;
        int next; // = 0;
        final double[] yrange = new double[2];
        final RawLinkArrayList subcurves = new RawLinkArrayList(0);
        RawChainArrayList chains = new RawChainArrayList(0);
        final RawLinkArrayList links = new RawLinkArrayList(0);
        ...
        while (left < numedges) {
            ...
            for (cur = left; cur < right; cur++) {
                ...
                for (next = cur; next > left; next--) {
                    final EdgeX prevedge = edgelist[next - 1];
                    final int ordering = e.compareTo(prevedge, yrange);
            ...
            newRow();
            final double ystart = yrange[0];
            final double yend = yrange[1];
            for (cur = left; cur < right; cur++) {
                e = edgelist[cur];
                int etag;
                ...
                if (eq != 0) {
                    ...
                    final int origstate = getState();
                    etag = (origstate == AreaOpX.RSTAG_INSIDE ? AreaOpX.ETAG_EXIT : AreaOpX.ETAG_ENTER);
        ...
        finalizeSubCurves(subcurves, chains);
        final int numlinks = subcurves.size();
        final AreaXBody ret = new AreaXBody(null, numlinks);
        final CurveLinkX[] linklist = subcurves.getArray();
    ...
    public static void finalizeSubCurves(final RawLinkArrayList subcurves, final RawChainArrayList chains) {
        ...
        final ChainEndX[] endlist = chains.getArray();
        for (int i = 1; i < numchains; i += 2) {
            final ChainEndX open = endlist[i - 1];
            final ChainEndX close = endlist[i];
            final CurveLinkX subcurve = open.linkTo(close);
            if (subcurve != null)
    ...
    private static RawChainArrayList resolveLinks(final RawLinkArrayList subcurves, final RawChainArrayList chains, final RawLinkArrayList links) {
        int curchain = 0;
        int curlink = 0;
        final int numlinks = links.size();
        final int numchains = chains.size();
        links.ensureCapacity(numlinks + 2);
        chains.ensureCapacity(numchains + 2);
        final CurveLinkX[] linklist = links.getArray();
        final ChainEndX[] endlist = chains.getArray();
        ...
        final RawChainArrayList newChains = new RawChainArrayList(0);
        ...
        while (chain != null || link != null) {
            ...
            if (connectchains) {
                final CurveLinkX subcurve = chain.linkTo(nextchain);
                if (subcurve != null)
                    subcurves.add(subcurve);
                curchain += 2;
            ...
            if (connectlinks) {
                final ChainEndX openend = new ChainEndX(link, null);
                final ChainEndX closeend = new ChainEndX(Objects.requireNonNull(nextlink), openend); // new ChainEndX(nextlink, openend);
     ...
     */
    public static boolean obstructs(final double v1, final double v2, final int phase) {
