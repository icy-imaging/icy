...
public class PluginErrorReportFrame extends TitledFrame {
     ...
     * Create the panel.
     */
    PluginErrorReportFrame(final PluginDescriptor plugin, final String devId, final String title, final String message) {
        super("Bug report", true, true, true, true);

        panel = new PluginErrorReportPanel(plugin, devId, title, message);

        panel.reportButton.addActionListener(e -> {
            ...
            try {
                ...
                IcyLogger.error(PluginErrorReportFrame.class, ex, "Error while reporting error.");
            }
            finally {
                progressFrame.close();
            }
        ...
        panel.closeButton.addActionListener(e -> close());
